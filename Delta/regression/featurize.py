...
class SampleEntity():
    ...
    def get_feature(self):
        ...
        return np.hstack((self.node_type, np.array(self.encoded_input_tables), np.array([self.width, self.rows, self.total_cost])))
...
class Normalizer():
    ...
    def norm(self, x, name):
        ...
        return (np.log(x + 1 + 1e-6) - self._mins[name]) / (self._maxs[name] - self._mins[name] + 1e-6)
