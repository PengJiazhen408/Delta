...
test_number = [18, 27, 52, 82, 98]
TPCDS_TEST_QUERIES = [f'query{n}_{i}.sql' for n in test_number for i in [1,2,3]]
...
class TopK_Naive(object):
    """Params for run.BalsaAgent."""
    @classmethod
    def Params(cls):
        p = hyperparams.InstantiableParams(cls)
        p.Define('train_history', './train_history', 'train_history')
        p.Define('qo_name', 'JOBRandSplit_PostgresSim-pr3huz6o',
                 'qo_name')
        p.Define(
            'data_augment', False, 'data augment'
        )
        p.Define(
            'unique', True, 'data augment'
        )
        p.Define(
            'data_augment_unique', True, 'data augment'
        )
        p.Define(
            'data_augment_essence', False, 'data augment'
        )
        p.Define(
            'all_iters', 2, 'all iters'
        )
        p.Define(
            'seed', 3407, 'all iters'
        )
...
@balsa.params_registry.Register
class TopK_Augment_Essence(TopK_Augment):
    """Params for run.BalsaAgent."""

    def Params(self):
        p = super().Params()
        p.data_augment_essence = True
        return p
...
@balsa.params_registry.Register
class Baseline_JoRandSplit(BalsaParams):
...
class BaselineTPCDS(Baseline):
    ...
    def Params(self):
        ...
        p.db = "tpcds"
        p.query_glob = ['*.sql']
        p.query_dir = 'queries/tpc-ds'
        p.test_query_glob = TPCDS_TEST_QUERIES
        return p
...
@balsa.params_registry.Register  # keep
class TPCDS(SlowMinCardCostLrC):
...
@balsa.params_registry.Register  # keep
class JOBSlowSplit_PostgresSim(Balsa_JOBSlowSplit):
