...
Usage:
    ...
    WANDB_MODE=disabled python run.py --run JOBRandSplit_PostgresSim --local
...
FLAGS = flags.FLAGS
flags.DEFINE_string('run', 'Baseline', 'Experiment config to run.')
...
def save_history(plan, query_name, history_dir, is_test, buffer_states=None, iter_num=None, is_balsa_top1=False):
    """save_history.
        Save the plan. 
        Args:
        ...
        IF in training phase:
            The location where the file is kept is: history_dir/iter_num/queryName_planSignatrueHash/plan
        IF in test phase:
    ...
    if is_test:
        if iter_num is None:
            history_q_path = os.path.join(history_dir, query_name)
        else:
            history_q_path = os.path.join(history_dir, str(iter_num), query_name)
        ...
        history_iter_path = os.path.join(history_dir, \
            f'{experiment}-{agent.wandb_logger.experiment.id}', f"{agent.curr_value_iter}")
    ...
    if is_balsa_top1:
            with open(os.path.join(history_plan_path, "is_balsa_top1"), "w") as f:
                f.write("Yes")
...
def ParseExecutionResult(result_tup,
    ...
    if engine == 'dbmsx':
        ...
        if has_timeout:
            ...
            planning_time = json_dict['Planning Time']
            save = True
            if save and not is_cached_plan:
                train_history_dir = "train_history"
                test_history_dir = "test_history_k"
                ...
                save_history(json_dict, query_name, history_dir, is_test, buffer_states, iter_num, is_balsa_top1)
    ...
    if hint_str is None or silent:
        ...
        return result_tup, real_cost, planning_time, server_ip, '\n'.join(messages)
...
class BalsaAgent(object):
    ...
    def __init__(self, params):
        ...
        self.prepare_elasped_time = []
        self.train_elasped_time = []
        self.test_elasped_time = []
        ...
        if p.use_local_execution:
            ray.init(resources={'pg': 1}, ignore_reinit_error=True)
        else:
    ...
    def _InitLogging(self):
        ...
        self.loggers = [
            ...
            pl_loggers.WandbLogger(save_dir=os.getcwd(), project='balsa-1520', name=experiment),
        ]
    ...
    def RunBaseline(self):
        ...
        for i, node in enumerate(self.all_nodes):
            ...
            result_tup = ExecuteSql(**Args(node))
            assert isinstance(
                result_tup,
                (pg_executor.Result, dbmsx_executor.Result)), result_tup
            result, real_cost, planning_time, _, message = ParseExecutionResult(
                result_tup, buffer_states, False, False, **Args(node))
            # Save real cost (execution latency) to actual.
            node.cost = real_cost
            node.actual_time_ms = real_cost
            node.info['pg_planning_time'] = planning_time
            print('---------------------------------------')
    ...
    def PlanAndExecute(self, model, planner, is_test=False, max_retries=3):
        ...
        for i, (kwarg, exec_result) in enumerate(zip(kwargs, exec_results)):
            ...
            result_tups = ParseExecutionResult(result_tup, buffer_states, is_cached_plan, False, **kwargs[i])
            assert len(result_tups) == 5
            print(result_tups[-1])  # Messages.
            execution_results.append(result_tups[:-1])
            # Increment counts for training.
    ...
    def FeedbackExecution(self, to_execute, execution_results):
        ...
        for node, result_tup, to_execute_tup in zip(self.train_nodes,
                                                    execution_results,
                                                    to_execute):
            result, real_cost, _, server_ip = result_tup
            ...
            baseline_total_latency += node.cost
            iter_max_latency = max(iter_max_latency, real_cost)
            self.LogScalars([
                ('latency/q{}'.format(node.info['query_name']), real_cost / 1e3,
                 self.curr_value_iter),
                ('relative_latency/q{}'.format(node.info['query_name']), real_cost / node.cost,
                 self.curr_value_iter),
        ...
        self.LogScalars([
            ...
            # 
            ('relative_latency/workload'.format(node.info['query_name']), iter_total_latency / baseline_total_latency,
                 self.curr_value_iter),
            # X-axis.
            ('curr_value_iter', self.curr_value_iter, self.curr_value_iter),
        ])
    ...
    def _SaveReplayBuffer(self, iter_total_latency):
        ...
        path = f'data/{experiment}-{self.wandb_logger.experiment.id}/'
        os.makedirs(path, exist_ok=True)
        path += \
            f'replay-{self.curr_value_iter}iters-{self.num_query_execs}execs-{len(self.exp.nodes)}nodes-{int(iter_total_latency / 1e3)}s.pkl'
        self.exp.Save(path)
        # Remove previous.
    ...
    def LogTestExperience(self,
        ...
        for node, to_execute, result_tup in zip(self.test_nodes,
                                                to_execute_test,
                                                execution_results):
            _, real_cost, _, _ = result_tup
            if real_cost < 0:
                has_timeouts = True
                break
            iter_total_latency += real_cost
            workload_total_latency += node.cost
            relative_lentcy.append(real_cost/node.cost)
            rows.append((node.info['query_name'], real_cost / 1e3,
                         real_cost/node.cost, self.curr_value_iter))
            data.append(('{}/q{}'.format(tag, node.info['query_name']),
                         real_cost / 1e3, self.curr_value_iter))
            data.append(('relative_latency_test/q{}'.format(node.info['query_name']),
                         real_cost / node.cost, self.curr_value_iter))
        ...
        table = wandb.Table(columns=['query_name', tag, 'relative_latency_test', 'curr_value_iter'],
                            rows=rows)
        self.wandb_logger.experiment.log({'{}_table'.format(tag): table})
        gmrl = np.exp(np.mean(np.log(relative_lentcy)))
        data.extend([
            (tag + '/workload', iter_total_latency / 1e3, self.curr_value_iter),
            ('relative_latency_test/workload', iter_total_latency / workload_total_latency, self.curr_value_iter),
            ('relative_latency_test/gmrl', gmrl, self.curr_value_iter),
    ...
    def RunOneIter(self):
        ...
        train_end = time.time()
        self.train_elasped_time.append(train_end-train_start)
        print("[Train] cur: {}s, all: {}s".format(train_end-train_start, sum(self.train_elasped_time)))
        ...
        to_execute, execution_results = self.PlanAndExecute(model,
                                                            ...
                                                            is_test=False)
        pre_end = time.time()
        self.prepare_elasped_time.append(pre_end-pre_start)
        print("[preparing training data] cur: {}s, all: {}s".format(pre_end-pre_start, sum(self.prepare_elasped_time)))
    ...
    def SaveAgent(self, model, iter_total_latency):
        ...
        experiment = str(p.cls).split('.')[-1][:-2]
        ckpt_dir = f'qo_models/{experiment}-{self.wandb_logger.experiment.id}/'
        os.makedirs(ckpt_dir, exist_ok=True)
        ckpt_path = os.path.join(ckpt_dir, 
                                 'checkpoint_{}.pt'.format(self.curr_value_iter))
        torch.save(model.state_dict(), ckpt_path)
        print('Saved iter={} checkpoint to: {}'.format(self.curr_value_iter,
                                                       ckpt_path))
    ...
    def EvaluateTestSet(self, model, planner, tag='latency_test'):
        ...
        test_end = time.time()
        self.test_elasped_time.append(test_end-test_start)
        print("[Test] cur: {}s, all: {}s".format(test_end-test_start, sum(self.test_elasped_time)))
    ...
    def LogTimings(self):
        ...
        data_to_log.extend([
            # Time, this iter.
            ('timing/train_all', iter_total_s, self.curr_value_iter),
            # Total time since beginning.
            ('timing_cumulative/train_all', total_s, self.curr_value_iter),
        ])
        ...
        if p.test_query_glob is not None and \
           ...
           num_iters_done % p.test_every_n_iters == 0:
            ...
            test_stages = stages + ['plan_test_set', 'wait_for_executions_test_set']
            timings = [self.timer.GetLatestTiming(s) for s in test_stages]
            cumulative_timings = [self.timer.GetTotalTiming(s) for s in test_stages]
            for stage, timing, cumulative_timing in zip(test_stages, timings,
                                                    cumulative_timings):
                data_to_log.extend([
                    # Time, this iter.
                    ('timing/{}'.format(stage), timing, self.curr_value_iter),
