...
#         assert stage not in self.start.keys(), f'{stage}: Forgot to call Stop()?'
#         curr_stage_start = time.time()
#         self.start[stage] = curr_stage_start

#     def Stop(self, stage):
#         assert stage in self.start.keys(), f'{stage}: Forgot to call Start()?'
#         curr_stage_start = self.start[stage]
#         self.start.pop(stage)
#         self.stage_timing_dict[stage].append(time.time() -
#                                              curr_stage_start)
        
#     def GetLatestTiming(self, stage):
#         if self.stage_timing_dict[stage]:
#             return self.stage_timing_dict[stage][-1]
#         else:
#             print('No timing info yet: ' + stage)
#             return 0.0
