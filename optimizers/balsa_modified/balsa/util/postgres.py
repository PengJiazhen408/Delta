...
ORDER BY table_name;
    ...
    indexes = []
    with pg_executor.Cursor() as cursor:
        results = pg_executor.Execute(sql, cursor=cursor).result
        for r in results:
            r = r[0]
            relations.append(r)
...
def GetTblSize(rel_or_idx):
    sql = f'select pg_table_size(\'{rel_or_idx}\')/8192;' 
    with pg_executor.Cursor() as cursor:
        pages_all = pg_executor.Execute(sql, cursor=cursor).result[0][0]
    return pages_all

def GetOsCacheState(rel_or_idx):
    sql = f'select * from pgfincore(\'{rel_or_idx}\');'
    with pg_executor.Cursor() as cursor:
        result = pg_executor.Execute(sql, cursor=cursor).result
    rel_os_pages = 0
    pages_mem = 0
...
def GetPgBufferState():
    ...
    for i, t in enumerate(result):
        ...
        pages_all = GetTblSize(rel_or_idx)
...
def GetBufferState():
    p = False
    relations, indexes = GetRelsAndIdx()
    print(f"Read {len(relations)} relations and {len(indexes)} indexes")
    
    pg_buffer_state = GetPgBufferState()
    ...
    for i in indexes:
        ...
        rel_os_pages, pages_mem = GetOsCacheState(i)
        if p:
    ...
    for r in relations:
        ...
        rel_os_pages, pages_mem = GetOsCacheState(r)
        if p:
            print(f"[OS]: {rel_os_pages}, {pages_mem}, {pages_mem/rel_os_pages * 100: .2f}%")
...
def ParsePostgresPlanJson(json_dict):
    ...
    def _parse_pg(json_dict, select_exprs=None, indent=0):
        ...
        if 'Scan' in op and select_exprs and op != "Bitmap Index Scan":
